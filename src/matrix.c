...
int matrix(WINDOW *mainwin) {
	int s = 60;
	int row_size, cols_size;//, r, c;
	get_term_size(mainwin, &cols_size, &row_size);
	start_color();
	...
	wclear(mainwin);
	print_massege(mainwin, cols_size, row_size);
	...
	while(true) {
		wtimeout(mainwin, s);
		switch(getch()) {
		case 'w': if (s > 10) s -= 10; break;
		case 's': if (s < 150) s += 10; break;
		}
		if (getch() == 'q') break;
		wclear(mainwin);
		for (int i = 0; i < row_size; i++) {
